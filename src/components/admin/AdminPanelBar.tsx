...
const AdminPanelBar = ({ content }: AdminPanelBarProps) => {
    ...
    async function loadReleases() {
      try {
        const resp = await fetch(
          "https://api.github.com/repos/aomtest/komari-slim-server/releases?per_page=100",
          {
            headers: {
  ...
  // 根据路径自动展开子菜单（包含动态扩展项），子菜单匹配基于 pathname 部分
  useEffect(() => {
          ...
          >
            <Flex gap="3" align="center">
              ...
              <a href="/" target="_blank" rel="noopener noreferrer">
                <label className="text-xl font-bold">komari-slim</label>
              </a>
